...
function renderMap() {
  ...
  CHAPTERS.forEach(ch => {
    const mine = chapterLevels(ch.id);
    ...
    h += `<div class="card chap ${open ? '' : 'locked'}"><div class="head">${sceneThumb(ch.bg)}<div class="grow"><div class="eyebrow">${esc(t('chapter'))} ${ch.id} · ${esc(tt(ch.card))}</div><h3>${esc(tt(SCENES[ch.bg].name))}</h3>
      <span class="faint">${open ? starsHTML(Math.min(3, Math.ceil(st / Math.max(1, mine.length))), 3) + ' ' + st + '/' + mine.length * 3 : '🔒 ' + esc(t('needStars', { n: ch.need }))}</span></div>${S.chests[ch.id] ? '<span class="chip gold">' + icon('chest') + '</span>' : ''}</div>`;
    h += '<div class="lvls">';
    mine.forEach(lvl => {
      const id = lvl.id, n = lvl.n, st = levelStars(id), isOpen = levelOpen(id);
