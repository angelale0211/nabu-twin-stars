...
function drawPics() {
  ...
  ['a', 'b'].forEach(side => {
    ...
    const svg = wrap.querySelector('svg.pic');
...
function onTap(e) {
  const g = GAME; if (!g.on || g.paused || g.over) return;
  e.preventDefault();
  const svg = e.currentTarget, pt = tapToPic(svg, g.lvl, e.clientX, e.clientY);
  ...
  if (hit) {
    ...
    const wrap = svg.parentNode, box = svg.getBoundingClientRect();
